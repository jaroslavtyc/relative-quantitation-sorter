...
class File extends Folder {
	...
	protected function setType() {
		$type = \filetype($this->getFullName());
		if ($type !== self::TYPE_FILE) {
			throw new Exceptions\WrongFolderType(sprintf('Expected [%s], given [%s].', self::TYPE_FILE, $this->type));
		} else {
			$this->type = $type;
		}
	}
	...
	protected function load()
	{
		if (!$this->fileExists())
			...
			$this->suffix = isset($info['extension'])
				? $info['extension']
				: FALSE;
			$this->filenameBody = $info['filename'];
